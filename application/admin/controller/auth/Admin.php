...
class Admin extends Backend
{
    ...
    public function recharge()
    {
        ...
        $result = \app\admin\model\Admin::naira($params['admin_id'],$params['money'], 1, 1,$this->auth->id);
        if(!$result){
    ...
    public function deduction()
    {
        ...
        if($params['money'] > $this->model->adminMoney($params['admin_id'])){
            $this->error(__('Insufficient administrator account balance'));
        }

        $params = $this->preExcludeFields($params);
        $result = \app\admin\model\Admin::naira($params['admin_id'],$params['money'], 2, 2,$this->auth->id);
        if(!$result){
            $this->error(__('Operation failed'));
        }
