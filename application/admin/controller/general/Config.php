...
class Config extends Backend
{
    ...
    protected $noNeedRight = ['check', 'rulelist', 'selectpage', 'get_fields_list','emailtest'];
    ...
    public function index()
    {
        ...
        foreach ($this->model->all() as $k => $v) {
            ...
            if($value['name'] == 'member'){
                $member = model('member')->order('id asc')->column('name,NGNUSD,USDCNY,NGNCNY','id');
                
                $value['value'] = $member;
                // [
                //     ['NGNUSD' => 1,'USDCNY'=>2,'NGNCNY'=>3],
                //     ['NGNUSD' => 1,'USDCNY'=>2,'NGNCNY'=>3],
                //     ['NGNUSD' => 1,'USDCNY'=>2,'NGNCNY'=>3],
                // ];
                // dd($value);
            ...
            if ($row) {
                ...
                try {
                    ...
                    if(isset($row['exchange'])){
                        foreach ($row['exchange'] as $key => $mval) {
                            model('member')->where('id',$key)->update($mval);
                        }
                    }
                } catch (Exception $e) {
                    $this->error($e->getMessage());
