<?php

return [
    'Name'                                 => '变量名',
    'Tip'                                  => '提示信息',
    'Group'                                => '分组',
    'Type'                                 => '类型',
    'Title'                                => '配置名',
    'Value'                                => '配置值',
    'BasicGroup'                           => '基础配置',
    'EmailGroup'                           => '邮件配置',
    'AttachmentGroup'                      => '附件配置',
    'DictionaryGroup'                      => '字典配置',
    'UserGroup'                            => '会员配置',
    'orderGroup'                           => '订单配置',
    'exchangeGroup'                        => '汇率设置',
    'Extend'                               => '扩展属性',
    'String'                               => '字符',
    'Password'                             => '密码',
    'Text'                                 => '文本',
    'Editor'                               => '编辑器',
    'Number'                               => '数字',
    'Date'                                 => '日期',
    'Time'                                 => '时间',
    'Datetime'                             => '日期时间',
    'Datetimerange'                        => '日期时间区间',
    'Image'                                => '图片',
    'Images'                               => '图片(多)',
    'File'                                 => '文件',
    'Files'                                => '文件(多)',
    'Select'                               => '列表',
    'Selects'                              => '列表(多选)',
    'Switch'                               => '开关',
    'Checkbox'                             => '复选',
    'Radio'                                => '单选',
    'Array'                                => '数组',
    'Array key'                            => '键名',
    'Array value'                          => '键值',
    'City'                                 => '城市地区',
    'Selectpage'                           => '关联表',
    'Selectpages'                          => '关联表(多选)',
    'Custom'                               => '自定义',
    'Please select table'                  => '关联表',
    'Selectpage table'                     => '关联表',
    'Selectpage primarykey'                => '存储字段',
    'Selectpage field'                     => '显示字段',
    'Selectpage conditions'                => '筛选条件',
    'Field title'                          => '字段名',
    'Field value'                          => '字段值',
    'Content'                              => '数据列表',
    'Rule'                                 => '校验规则',
    'Visible condition'                    => '可见条件',
    'Site name'                            => '站点名称',
    'Beian'                                => '备案号',
    'Cdn url'                              => 'CDN地址',
    'Version'                              => '版本号',
    'Timezone'                             => '时区',
    'Forbidden ip'                         => '禁止IP',
    'Languages'                            => '语言',
    'Fixed page'                           => '后台固定页',
    'Category type'                        => '分类类型',
    'Config group'                         => '配置分组',
    'Attachment category'                  => '附件类别',
    'Category1'                            => '分类一',
    'Category2'                            => '分类二',
    'Rule tips'                            => '校验规则使用请参考Nice-validator文档',
    'Extend tips'                          => '扩展属性支持{id}、{name}、{group}、{title}、{value}、{content}、{rule}替换',
    'Mail type'                            => '邮件发送方式',
    'Mail smtp host'                       => 'SMTP服务器',
    'Mail smtp port'                       => 'SMTP端口',
    'Mail smtp user'                       => 'SMTP用户名',
    'Mail smtp password'                   => 'SMTP密码',
    'Mail vertify type'                    => 'SMTP验证方式',
    'Mail from'                            => '发件人邮箱',
    'Site name incorrect'                  => '网站名称错误',
    'Name already exist'                   => '变量名称已经存在',
    'Add new config'                       => '点击添加新的配置',
    'Send a test message'                  => '发送测试邮件',
    'Only work at development environment' => '只允许在开发环境开操作',
    'This is a test mail content'          => '这是一封来自%s的校验邮件,用于校验邮件配置是否正常!',
    'This is a test mail'                  => '这是一封来自%s的邮件',
    'Please input your email'              => '请输入测试接收者邮箱',
    'Please input correct email'           => '请输入正确的邮箱地址',
    'Charge for trouble one(fixed)'        => '手续费-1(奈拉)(固定)',
    'Charge for trouble one(permillage)'   => '手续费-1(奈拉)(千分比)',
    'Charge for trouble two'               => '手续费-2',
    'Please enter the handling fee'        => '请输入手续费',
    'Bank charges' => '银行手续费(奈拉)',
    'Please enter the bank handling fee' => '请输入银行手续费',

    'Member exchange rate'                 => '会员汇率(会员等级由低到高)',

    'Exchange rate of RMB'                 => 'RMB汇率',
    'Exchange rate of USDT'                 => 'USDT汇率',
    'System configuration'                 => '系统配置',
    'Customer service email'               => '客服邮箱',
    'Please enter customer service email' => '请输入客服邮箱',
    'Customer service mobile'              => '客服电话',
    'Please enter customer service mobile' => '请输入客服电话',
    'Customer service qrcode'              => '客服二维码',
    'Please upload the customer service QR code' => '请上传客服二维码'
];
