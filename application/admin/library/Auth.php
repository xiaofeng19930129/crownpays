...
class Auth extends \fast\Auth
{
    ...
    protected function keeplogin($admin, $keeptime = 0)
    {
        if ($keeptime) {
            ...
            Cookie::set('keeplogin', implode('|', [$admin['id'], $keeptime, $expiretime, $key]), $keeptime);
            Cookie::set('keeplogin1', $keeptime, $keeptime);
            // dd([Cookie::get('keeplogin'),Cookie::get('keeplogin1')]);
            return true;
        }
        return false;
    }
    ...
    public function getAdminNaira(){
        return Admin::where('id',$this->id)->value('naira');
    }
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        if (Config::get('fastadmin.multiplenav')) {
            ...
            foreach ($topList as $index => $item) {
                ...
                $nav .= '<li class="' . ($current ? 'active' : '') . '"><a ' . $item['extend'] . ' href="' . $url . $addtabs . '" ' . $item['menutabs'] . ' class="' . $item['menuclass'] . '" url="' . $url . '" title="' . $item['title'] . '"><i class="' . $item['icon'] . '"></i> <span>' . __($item['title']) . '</span> <span class="pull-right-container"> </span></a> </li>';
                $menu .= $childList;
            }
        } else {
            // 构造菜单数据
            ...
            if ($selected) {
                $nav .= '<li role="presentation" id="tab_' . $selected['id'] . '" class="' . ($referer ? '' : 'active') . '"><a href="#con_' . $selected['id'] . '" node-id="' . $selected['id'] . '" aria-controls="' . $selected['id'] . '" role="tab" data-toggle="tab"><i class="' . $selected['icon'] . ' fa-fw"></i> <span>' . __($selected['title']) . '</span> </a></li>';
            }
            if ($referer) {
                $nav .= '<li role="presentation" id="tab_' . $referer['id'] . '" class="active"><a href="#con_' . $referer['id'] . '" node-id="' . $referer['id'] . '" aria-controls="' . $referer['id'] . '" role="tab" data-toggle="tab"><i class="' . $referer['icon'] . ' fa-fw"></i> <span>' . __($referer['title']) . '</span> </a> <i class="close-tab fa fa-remove"></i></li>';
            }
        }
