...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'usdt','naria','member_id','deposit','current_naira','total_current_naira','code','security'];
    ...
    public function init($token)
    {
        ...
        if ($user_id > 0) {
            ...
            $member = \app\common\model\Member::get($user['member_id']);
            $user['member'] = $member;
            $this->_user = $user;
            $this->_logined = true;
            $this->_token = $token;
    ...
    public function getUserinfo()
    {
        ...
        $tokenUser = Token::get($this->_token);
        unset($tokenUser['user_id']);
        $userinfo = array_merge($userinfo, $tokenUser);
        return $userinfo;
    }
    ...
    {
        return $this->_errorCode ? $this->_errorCode : 0;
    }
}
