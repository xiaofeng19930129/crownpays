...
use app\common\model\OrderShare;
use think\Config;
use think\Cookie;
use think\Hook;
use think\Session;
...
class User extends Mobile
{
    ...
    public function register()
    {
        ...
        if ($this->auth->id) {
            $this->success(__('You\'ve logged in, do not login again'), $url ? $url : url('mobile/user/index'));
        }
        if ($this->request->isPost()) {
            ...
            if(!empty($code)){
                if(!$pid = $userModel->checkCode($code)){
                    $this->error(__('Invitation code error'));
                }
            }
            ...
            if ($this->auth->register($username, $password, $email, $mobile,['code' => $userCode,'pid'=>$pid])) {
                $this->success(__('Sign up successful'), $url ? $url : url('mobile/user/index'));
            } else {
    ...
    public function login()
    {
        $url = $this->request->request('url', '', 'url_clean');
        if ($this->auth->id) {
            $this->success(__('You\'ve logged in, do not login again'), $url ?: url('mobile/user/index'));
        }
        if ($this->request->isPost()) {
            ...
            if ($this->auth->login($account, $password)) {
                $this->success(__('Logged in successful'), $url ? $url : url('mobile/user/index'));
            } else {
                $this->error($this->auth->getError(), null, ['token' => $this->request->token()]);
            }
        }
    ...
    public function logout()
    {
        if ($this->request->isPost()) {
            ...
            //退出本站
            $this->auth->logout();
            $this->success(__('Logout successful'), url('mobile/user/index'));
        }
    ...
    public function order(){
        ...
        $orderList = (new Order)->getOrderList($this->auth->id,$page,$time);
    ...
    public function member(){
        $nextLevel = (new Member)->getNextLevel($this->auth->member->level);
    ...
    public function agent(){
        $agentCensus = (new UserModel)->agentCensus($this->auth->id);
        $shareOrderList = (new OrderShare())->getOrderShareList($this->auth->id);
        $this->assign('agentCensus',$agentCensus);
        $this->assign('shareOrderList',$shareOrderList);
        return $this->view->fetch();
    }
    ...
    public function cash(){
        $cashList = (new UserCash)->getCashList($this->auth->id);
